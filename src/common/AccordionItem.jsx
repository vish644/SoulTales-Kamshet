...
export default function AccordionItem({ question, answer, isOpen, onToggle }) {
  return (
    ...
    >
      ...
      >
        <span className="font-body font-bold text-[15px] md:text-base text-black leading-6 tracking-normal">
          {question}
        </span>
      ...
      {isOpen && (
        <div className="px-6 pb-6 -mt-1 space-y-4">
          {answer.map((paragraph, i) => (
            <p
              key={i}
              className="text-[14px]! md:text-p! leading-6 font-body font-normal  tracking-normal"
            >
              {paragraph}
            </p>
          ))}
